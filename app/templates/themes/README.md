...
**请不要直接修改官方默认主题（如 `spa`, `default`, `next` 等）的文件！**

流光笔记支持一键自动更新。在系统更新时，官方自带的主题文件会被最新版本**直接覆盖**。如果您直接在默认主题上进行了修改，且未提PR推送，（例如修改了 CSS 样式、HTML 结构或替换了图片），这些修改将在更新后**全部丢失**。

**正确的自定义主题方式：**
1. 复制一个您喜欢的默认主题文件夹（例如将 `themes/spa` 复制并重命名为 `themes/my_theme`）。
2. 在新文件夹 `my_theme` 中进行您的所有自定义修改。
3. 在后台设置中，将前台主题切换为您新建的 `my_theme`。

> 系统更新时，只会覆盖官方同名文件，您自定义的主题文件夹（如 `my_theme`）是**绝对安全**的，不会被删除或覆盖。
